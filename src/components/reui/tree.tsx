...
function TreeItem<T = any>({
  ...
  const defaultProps = {
    ...
    style: mergedStyle,
    className: cn(
      "z-10 ps-(--tree-padding) outline-hidden select-none focus:z-20 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 relative",
      className
    ),
    "data-focus":
...
function TreeDragLine({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <div
      style={dragLine}
      className={cn(
        "bg-primary before:bg-background before:border-primary absolute z-30 -mt-px h-0.5 before:absolute before:-top-[3px] before:left-0 before:size-2 before:border-2",
        "before:rounded-full",
        className
      )}
      {...props}
