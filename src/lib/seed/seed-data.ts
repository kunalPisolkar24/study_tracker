...
export const SPREAD_START = new Date("2026-04-01");
export const SPREAD_END = new Date("2026-06-28");
export const SPREAD_DAYS = Math.max(1, Math.round((SPREAD_END.getTime() - SPREAD_START.getTime()) / 86_400_000));

export function buildSeedNodes(): SeedNodeDef[] {
  const CN = "seed-ws-cn";
  const OS = "seed-ws-os";
  ...
  function n(id: string, workspaceId: string, title: string, orderIndex: number, overrides?: Partial<Omit<SeedNodeDef, "id" | "workspaceId" | "title" | "orderIndex">>): SeedNodeDef {
    return { id, workspaceId, parentId: null, title, orderIndex, status: null, confidence: null, notes: "", lastReviewedAt: null, createdAt: "2026-01-01T00:00:00.000Z", ...overrides };
  }

  function c(id: string, parentId: string, workspaceId: string, title: string, orderIndex: number, overrides?: Partial<Omit<SeedNodeDef, "id" | "parentId" | "workspaceId" | "title" | "orderIndex">>): SeedNodeDef {
